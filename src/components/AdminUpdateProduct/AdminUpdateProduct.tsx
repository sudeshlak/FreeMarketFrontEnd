...
const AdminUpdateProduct: React.FC<AdminUpdateProductProps> = (props) => {
  ...
  const handleOnSubmit = async (event: FormEvent) => {
    ...
    if (!productNewImage) {
      ...
      uploadToS3(url.data.generatePutUrl, productNewImage, productImageName)
  ...
  useEffect(() => {
    ...
    setCategory({
      value: props.updateToProduct.category.id,
      label: props.updateToProduct.category.title,
    });
      ...
      <div className="py-4">
        <Form onSubmit={handleOnSubmit}>
          ...
          <Form.Group controlId="formFileSm" className="mb-3">
            ...
            />
            <Row>
              <span className="error-message">{fileError && submit}</span>
            </Row>
          </Form.Group>
          <Button
            variant="primary"
