...
const AdminUpdateProductPreview: React.FC<AdminUpdateProductPreviewProps> = (
  props,
) => {
  const [displayImage, setDisplayImage] = useState<string>(insertImage.src);
  ...
  useEffect(() => {
    if (!props.productNewImage && !props.updateToProduct?.image) {
      setDisplayImage(insertImage.src);
      return;
    }
  ...
  return (
    <React.Fragment>
      <Col xs={12} className="update-product-preview-title">
        Preview{" "}
      </Col>
      <Col className="product px-2 " lg="3" md="4" xs="6">
        <Row className="product-item py-2">
          <Col className="img-col" xs="12">
            {props.productNewImage ? (
              <Image className="img" src={displayImage} alt="Image" />
            ) : (
