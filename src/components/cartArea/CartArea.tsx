...
const CartArea: React.FC = () => {
  ...
  const subTotalPrice = cartItems.reduce(
    (total: number, b: IProduct) =>
      total + (b.regular_price - b.discount_price) * b.quantity,
    0,
  );
  ...
  const RenderCartArea = () => {
    if (!cartItems || cartItems.length === 0) {
      return (
        <div className="empty-cart" data-test="empty-cart">
          <Col xs={12} className="px-4 text-center">
            <Image src={emptyCart.src} className="empty-cart-image" />
          </Col>
          <Col xs={12} className="empty-cart-row-1">
            Your Cart is empty
          </Col>
          <Col xs={12} className="empty-cart-row-2">
      ...
      return (
        <div className="non-empty-cart" data-test="non-empty-cart">
          ...
          <Row className="mx-1">
            <Col>
              ...
              <button
                ...
                data-test="cart-checkout-btn"
                onClick={onclickRouteCheckout}
              >
                Checkout
              </button>
            </Col>
  ...
  return <React.Fragment>{RenderCartArea()}</React.Fragment>;
};
