...
const CreateProductForm: React.FC<CreateProductFormProps> = (props) => {
  ...
  const [generatePutUrl] = useMutation<{ generatePutUrl: string }>(
    GENERATE_PUT_URL,
  );
  ...
  const handleOnSubmit = async (event: FormEvent) => {
    ...
    const url: FetchResult = await fetchGeneratePutUrl(
      productImageName,
      "jpeg",
    );
    if (!url || !url.data) {
    ...
    uploadToS3(url.data.generatePutUrl, productImage, productImageName)
      .then((value: AxiosResponse) => {
        addProductToDatabase(
          ...
          .then(({ data }) => {
            ...
            toast(
              "New Product: " + productName + " created successfully!",
              "",
              "success",
            );
            clearForm();
      ...
      <Form onSubmit={handleOnSubmit}>
        ...
        <Row>
          <Form.Group as={Col} xs={12} md={6} controlId="price">
            ...
            />
            <Row>
              <span className="error-message">{priceError && priceError}</span>
            </Row>
          </Form.Group>
          <Form.Group as={Col} xs={12} md={6} controlId="price">
        ...
        <Form.Group controlId="formFileSm" className="mb-3">
          ...
          <Form.Control
            ...
            size="sm"
            onChange={(event) =>
              fileChange((event.target as HTMLInputElement).files)
            }
            key={key || ""}
          />
          <Row>
            <span className="error-message">{fileError && fileError}</span>
          </Row>
        </Form.Group>
        <Button
          variant="primary"
