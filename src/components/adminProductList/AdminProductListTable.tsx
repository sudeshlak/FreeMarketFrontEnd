...
import {
  ...
  ISearchedCategory,
} from "../../types/AdminProductListType";
...
const AdminProductListTable: React.FC<AdminProductListTableProps> = (props) => {
  const [deleteProduct] = useMutation<{ deleteProduct: IProduct }>(
    DELETE_PRODUCT,
  );
  ...
  const listRows = () => {
    ...
    return props.categorizedItem.map((item: IProduct, index: number) => {
      const itemRow: any = {
        ...
        name: item.title,
        image: (
          <DisplayImage image={item.image} className={"cart-product-image"} />
        ),
        ...
        removeIcon: (
          <Trash
            ...
            color="red"
            onClick={() =>
              handleOnDeleteProduct(item.id, item.title, item.image)
            }
          />
        ),
      };
  ...
  return (
    <Card.Body className="admin-product-list pt-0">{getTable()}</Card.Body>
  );
};
