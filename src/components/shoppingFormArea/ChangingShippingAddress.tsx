...
const ChangingShippingAddress: React.FC<ChangingShippingAddressProps> = (
  props,
) => {
  ...
  const handleOnChangeAddress = (address: string) => {
    dispatch(
      changeFormData({ key: "otherAddressBillingAddress", value: address }),
    );
    ...
    dispatch(
      changeFormData({ key: "otherAddressBillingAddressError", value: "" }),
    );
  };
  ...
  const handleOnChangePostalCode = (postalCode: string) => {
    dispatch(
      changeFormData({ key: "otherAddressPostelCode", value: postalCode }),
    );
    if (!isNotEmpty(postalCode)) {
      dispatch(
        changeFormData({
          key: "otherAddressPostelCodeError",
          value: "Enter valid postal code",
        }),
      );
      return;
    }
    ...
    if (!validateOnlyNumbers(contactNumber)) {
      dispatch(
        changeFormData({
          ...
          value: "Enter valid contact number",
        }),
      );
      return;
    }
    dispatch(
      changeFormData({ key: "otherAddressContactNumberError", value: "" }),
    );
  };
  ...
  const countrySelect: CountrySelect[] = Country.map((country) => {
    return { value: country.value, label: country.label };
  });

  const countryCode = () => {
    if (
      shippingForm.otherAddressCountry?.value === "United States of America"
    ) {
      return (
        <div>
      ...
      return (
        <div>
          <img src="https://cdnjs.cloudflare.com/ajax/libs/flag-icon-css/3.4.3/flags/4x3/as.svg" />
      ...
      return (
        <div>
          <img src="https://cdnjs.cloudflare.com/ajax/libs/flag-icon-css/3.4.3/flags/4x3/lk.svg" />
      ...
      return (
        <div>
          <img src="https://cdnjs.cloudflare.com/ajax/libs/flag-icon-css/3.4.3/flags/4x3/sg.svg" />
      ...
      return (
        <div>
          <img src="https://cdnjs.cloudflare.com/ajax/libs/flag-icon-css/3.4.3/flags/4x3/in.svg" />
