...
const DisplayImage: React.FC<DisplayImageProps> = (props) => {
  ...
  const [generateGetUrl] = useMutation<{ generateGetUrl: string }>(
    GENERATE_GET_URL,
  );
  const fetchGenerateGetUrl = React.useCallback(
  ...
  return (
    <React.Fragment>
      {image && (
        <img src={image} alt="image not found" className={props.className} />
      )}
    </React.Fragment>
  );
};
