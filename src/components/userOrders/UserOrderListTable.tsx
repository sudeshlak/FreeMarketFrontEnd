...
const UserOrderListTable: React.FC<OrderListProps> = (props) => {
  const [getOrdersByToken] = useMutation<{ getOrdersByToken: IOrder[] }>(
    CHANGE_ORDER_BY_TOKEN,
  );
  ...
  const listRow = React.useCallback(() => {
    if (props.activeStatus.label === "All") {
      ...
      return userOrders
        .filter(
          (order: IOrder) =>
            order.status === props.activeStatus.label.toLowerCase(),
        )
        .map((order: IOrder) => {
          return {
            key: order.id,
  ...
  return (
    <Row>
      <Card.Body className="user-order-list">{getTable()}</Card.Body>
    </Row>
  );
};
